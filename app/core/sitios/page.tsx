...
const SitesPage = () => {
  ...
  const { data: sites, error, loading } = useFetch({ url: "/sitios" });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire("Error", "Ocurrió un problema al eliminar la bodega", "error");
        }
      }
    });
  };
  ...
  useEffect(() => {
    if (sites) {
      const pages = Math.ceil(sites.length / 3);
      setTotalPages(pages);
    }
  }, [sites]);
